...
class VanillaOptionPricerTest:
    ...
    def __init__(self,
        ...
        if not self.dividend_schedule:
            ...
            self.dividend_schedule_ql = [ql.FixedDividend(amount, pay_date)
        ...
        self.dividend_curve = ql.FlatForward(0, self.calendar, 0.0, self.day_counter,
                                             ql.Continuous, ql.Annual)
        # risk-free/carry curve used inside PDE
        self.carry_curve    = ql.FlatForward(0, self.calendar, self.carry_rate,
                                             self.day_counter, ql.Continuous, ql.Annual)
        self.volatility_curve = ql.BlackConstantVol(0, self.calendar,
                                                    volatility, self.day_counter)

        # (optional) a handle to your discount zero curve (not used by solver)
        self.yield_curve = self.build_quantlib_yield_curve_ts(self.discount_curve)

        # effective S used by process (cash vs physical alignment you already had)
        ...
        tau_carry_start_to_carry_end = self.day_counter.yearFraction(self.carry_start,
                                                                     self.carry_end)
        # back out flat dividend yield that reproduces PV(divs) over [carry_start, carry_end]
        ...
        # --- process / payoff / exercise / vanilla option shell (for payoff only) ---
        self.process = ql.BlackScholesMertonProcess(
            ql.QuoteHandle(self.underlying),
            ql.YieldTermStructureHandle(self.dividend_curve),  # =0 to avoid double count
        ...
        self.option = ql.VanillaOption(self.payoff, self.exercise)  # unused engine-wise

    # ---------------- utilities you already had (unchanged) ----------------
    ...
    def _add_nacc_and_dfs(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        data["NACC"] = data["QuantLib_Date"].apply(lambda d: self.get_nacc_rate(d))
    ...
    # ---------------- FIS-compliant mesher + time grid + backward solver ----------------

    def _div_times_from_eval(self) -> list[float]:
        """dividend pay times measured from evaluation date (t=0)."""
        ts = []
        for div in self.dividend_schedule_ql:
            d = div.date()
    ...
    def _build_time_grid(self, dx: float) -> ql.TimeGrid:
        """Piecewise grid with mandatory nodes at dividends and expiry; applies FIS Δt relation per segment."""
    ...
    def _build_solver(self, x_grid: int, scheme: ql.FdmSchemeDesc) -> ql.FdmBackwardSolver:
    ...
    # ---------------- pricing & greeks (now via custom solver) ----------------
    ...
    def _price_once(self, t_steps: int, scheme=ql.FdmSchemeDesc.CrankNicolson()):
        ...
        # one Rannacher start (global) — public Python API can’t restart at each dividend
        solver.rollback(self.time_to_discount, self._DAMPING_STEPS)
        pv_at_t0 = float(solver.valueAt(self.underlying.value()))

        # settlement window correction (your original logic retained)
    ...
    def plot_price_convergence(self, time_steps_list, style='seaborn-v0_8-darkgrid'):
        ...
        plt.title(f"{self.option_type_str} Option Price vs Time Steps ({self.exercise_type_str} Exercise)")
    ...
    def _clone(self, *, spot_price=None, volatility=None, valuation_date=None) -> "VanillaOptionPricerTest":
    ...
    def calculate_greeks(self, time_steps):
        ...
        bump_rel = 1e-4  # 1bp in relative S'
        if self.settlement_type == "cash":
            base = self.s_cash
            up   = self._clone();   up.s_cash   = (1 + bump_rel) * base; up.underlying.setValue(up.s_cash)
            dn   = self._clone();   dn.s_cash   = (1 - bump_rel) * base; dn.underlying.setValue(dn.s_cash)
        else:
            base = self.s_physical
            up   = self._clone();   up.s_physical = (1 + bump_rel) * base; up.underlying.setValue(up.s_physical)
            dn   = self._clone();   dn.s_physical = (1 - bump_rel) * base; dn.underlying.setValue(dn.s_physical)
    ...
    # --- reports unchanged (omitted for brevity in this snippet) ---
