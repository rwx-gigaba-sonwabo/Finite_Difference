...
class AmericanFDMPricer:
    ...
    Interface aligned with run_american_scenarios.py:
      - price_log2()
      - greeks_log2()

    No barrier logic – only American early exercise:
    ...
    def __init__(
        ...
        # Trade/meta info (not used directly in PDE)
        ...
        # --- Grid / CN controls (with safe minimums to prevent empty systems) ---
        self.grid_type = grid_type
        self.s_max_mult = float(s_max_mult)
        self.num_space_nodes = max(int(num_space_nodes), 3)   # at least 3
        self.num_time_steps = max(int(num_time_steps), 3)     # at least 3
    ...
    def _boundary_values(self, tau: float) -> Tuple[float, float]:
        ...
        Early exercise is handled by projection after each step.
        """
    ...
    def _solve_grid(self, N_time: Optional[int] = None) -> List[float]:
        ...
        plus American early exercise V_j = max(V_j, intrinsic(S_j)).
        """
        dx = self._build_log_grid()

        # Use actual grid length, not just num_space_nodes (more robust)
        N = len(self.s_nodes) - 1
        if N < 2:
            raise RuntimeError(
                f"Spatial grid too coarse: need at least 3 nodes, got {N+1}."
            )
        ...
        def solve_tridiag(A_L, A_C, A_U, rhs):
            n = len(rhs)
            if n == 0:
                raise RuntimeError("Tridiagonal system has zero size (no interior nodes).")
